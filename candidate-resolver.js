...
function parseCandidateData (candidate_input) {
...
async function createIbanHisttoryAndValidate (_id, payload) {
    const ibanHistory = await IbanHistoryModel.create(payload);

    try {
        await CandidateUtility.validateIbanBicCandidate(payload.iban, payload.bic);
        await logIban(ibanHistory._id, { message: 'success' })
    } catch (error) {
        await logIban(ibanHistory._id, { message: error })
...
async function UpdateCandidate(
  ...
  candidate_input = parseCandidateData(candidate_input)
  ...
  if (candidate_input.parents && candidate_input.parents.length) {
    for (let parent of candidate_input.parents) {
      if (parent.iban && parent.bic && parent.account_holder_name) {
        await createIbanHisttoryAndValidate(_id, {
            candidate_id: _id,
            iban: parent.iban,
            bic: parent.bic,
