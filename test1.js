...
function result(sessions) {
  ...
  sessions.forEach((session) => {
    const currentSession = session.session_id - 1;
    if (formattedSession[currentSession] === undefined) {
      formattedSession[currentSession] = {
    ...
    const currentClass = formattedSession[currentSession].classes.find(
      (cls) => cls.class_id === session.class.class_id
    );
    if (currentClass === undefined) {
      formattedSession[currentSession].classes.push({
    ...
    formattedSession[currentSession].classes[
      formattedSession[currentSession].classes.length - 1
    ].students.push({
